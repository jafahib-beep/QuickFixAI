...
// Auth (safe import from routes/auth.js)
...
const awardCommentXp = xp.awardCommentXp;
...
router.put("/:postId/comments/:commentId/solution", authMiddleware, async (req, res) => {
  try {
    ...
    await pool.query("UPDATE community_comments SET is_solution = false WHERE post_id = $1", [postId]);
    await pool.query("UPDATE community_comments SET is_solution = true WHERE id = $1", [commentId]);
    await pool.query("UPDATE community_posts SET status = 'solved', updated_at = NOW() WHERE id = $1", [postId]);
